...
2. Controller의 `Receivers` 목록에 Surface별 항목을 추가합니다.
3. `Surface Kind`를 선택하고 Static Mesh면 source `MeshFilter`/동일 Mesh의 `MeshCollider`를,
   Terrain이면 `Terrain`/동일 TerrainData의 `TerrainCollider`를 연결합니다.
4. 시각화가 필요하면 해당 Receiver의 Grid 출력 전용 `MeshFilter`와 `MeshRenderer`를 함께 연결합니다.
5. Receiver별 Seed Triangle index와 barycentric 좌표를 지정하고 `Rebuild Tiles`를 실행합니다.

Receiver의 Output 두 참조를 모두 비우면 논리 타일·피킹·상태만 사용하는 logical-only Grid로 동작합니다. 출력할
때는 Source와 output MeshFilter가 달라야 하며 Controller가 원본 Geometry 덮어쓰기를 검증합니다.
Receiver마다 topology·좌표·Tile 상태가 독립적이므로 서로 다른 Surface의 같은 `(q,r)`은 동일 Tile이
아닙니다. 한 Receiver가 잘못 구성돼도 유효한 나머지 Receiver는 계속 Bake됩니다.
...
- Skinned Mesh binding은 후속 단계입니다.
